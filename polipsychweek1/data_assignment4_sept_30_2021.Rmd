...
```{r}
pid3llabs <- c("1" = "Democrat", "2" = "Independent", "3" = "Republican")
sdo %>%
  drop_na(pid3) %>%
  ggplot(aes(x = sdo5)) +
  geom_histogram(fill = "magenta") + 
  facet_wrap(~ pid3, labeller = labeller(pid3 = pid3llabs) ) +
  labs(title = "SDO Frequency by Party ID") +
  xlab("SDO Value") + 
  ylab("Frequency")
...
givemodel <- lm(rep_vote ~ sdo5 + female + white + educ + age + pid3 +ideo5, sdo)
stargazer(givemodel, header = FALSE, type = "text")
...
```{r}
replacedsdo <- sdo %>% summarise(sdage = sd(age), 
            sdsdo5 = sd(sdo5),
            sdfemale = sd(female),
            sdwhite = sd(white),
            sdeduc = sd(educ),
            sdpid3 = sd(pid3, na.rm = TRUE),
            sdideo5 = sd(ideo5, na.rm = TRUE))
replacedsdo %>%  
  mutate(Age = sdage * 0.002078, 
         SDOrank = sdsdo5 * 0.093533,
         Female = sdfemale * -0.003586,
         White = sdwhite * 0.044000,
         Education = sdeduc * -0.011131,
         Partisan_Identity = sdpid3 * 0.248418,
         Ideology = sdideo5 * 0.108795) %>%
  select(-c(sdsdo5, sdfemale, sdwhite, sdeduc, sdpid3, sdideo5, sdage)) %>%
  pivot_longer(cols = everything(), names_to="Predictor", values_to="Strength_of_Coeff") %>%
  ggplot(aes(x = Predictor, y = Strength_of_Coeff)) +
...
library("stargazer")
sdo2 <- sdo %>% 
  mutate(confederate_flag = case_when(
  confederate_flag == "Heritage" ~ 1,
  confederate_flag == "Slavery" ~ 0, 
  T ~ NA_real_))

m1 <- lm(confederate_flag ~ fear_of_demographic_change, data = sdo2)

#stargazer(m1, header = FALSE, type = "text")
```
