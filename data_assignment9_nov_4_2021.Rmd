...
Study1_processing1 <- Study1_preprocess %>% 
  rename_with(.cols = c(Q13, Q14, Q15), ~c("IDTransmit", "IDCure", "InfoReq"))
```
...
Study1_processing2 <- Study1_processing1 %>% 
  rename_with(.cols = last_col(offset = 5):last_col(), ~c("gender", "race", "education", "party id", "voter_reg_status", "ideology"))
...
Study1_processing3 <- Study1_processing2 %>% 
  rename_with(.cols = contains("Q11"), ~c("Disgust", "Grossed Out", "Reupulsed", "Afraid", "Anxious", "Worried"))
...
Study1_processing5 <- Study1_processing4 %>% 
  mutate(across(.cols = c("Disgust", "Grossed Out", "Reupulsed", "Afraid", "Anxious", "Worried", "IDTransmit", "IDCure", "InfoReq","Q17_6", "Q17_7"), ~na_if(.,8)))
...
Study1_processing6 <- Study1_processing5 %>%
  mutate(Q12_1= if_else(Q12_1==2, 1, 0), 
         Q12_2= if_else(Q12_2==2, 1, 0),
         Q12_3= if_else(Q12_3==2, 1, 0), 
         Q12_4= if_else(Q12_4 ==2, 1, 0),
         Q12_5= if_else(Q12_5==2, 1, 0),
         Q12_6= if_else(Q12_6==2, 1, 0),
         Q12_7= if_else(Q12_7==2, 1, 0),
         IDCure= if_else(IDCure==2, 1, 0),
         InfoReq= if_else(InfoReq==2, 1, 0))
...
```{r}
final_data <- Study1_processing6 %>%
  mutate(treatment = as_factor(case_when(treat_rand1 == 1 ~ 'Low Anxiety/Low Disgust',
                                         treat_rand1 == 2 ~ 'High Anxiety/Low Disgust',
                                         treat_rand1 == 3 ~ 'Low Anxiety/High Disgust',
                                         treat_rand1 == 4 ~ 'High Anxiety/High Disgust'))) %>%
  mutate(treat_anxious = as_factor(case_when(treat_rand1 == 1 ~ 0, treat_rand1 == 2 ~ 1,treat_rand1 == 3 ~ 0,
                                         treat_rand1 == 4 ~ 1)))
glimpse(final_data)
...
```
```{r}
alpha_data <- final_data %>% 
  select(c(Anxious, Disgust)) %>%
  drop_na()

alpha <- cronbach.alpha(data = alpha_data)
...
```{r}
reg_mod <- lm(InfoReq ~ gender + `party id`
              , data = final_data)
emot_mod <- lm(InfoReq ~ treat_anxious
              , data = final_data)
```
...
```{r}
stargazer(reg_mod, type = "text")
stargazer(emot_mod, type = "text")
