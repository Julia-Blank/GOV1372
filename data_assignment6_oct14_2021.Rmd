...
```{r}
mod2 <-lm(init ~ suffrage + polity, data = s_data)
tab_model(mod2)
...
```{r}
mod1 <- lm(init ~ suffrage + polity +polity*suffrage, data = s_data)
tab_model(mod1)

››```

```{r}
mod_3 <- lm(init ~ suffrage + polity + polity*suffrage,
            data = s_data)
mod_4 <- lm(init ~ suffrage + polity,
            data = s_data)
stargazer(mod_3, mod_4, 
          type = "latex")
```
