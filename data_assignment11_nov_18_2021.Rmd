...
misinfo <- load("misinfo.combined.RData")
...
misinfo.combined %>%
  group_by(topic) %>%
  drop_na() %>%
  summarize(mean(correct.answer))#, .groups = 'keep')
...
misinfo.combined %>%
  ...
  summarize(mean_correct = mean(correct.answer), .groups = 'keep') %>%
  mutate(number_correct = 5*mean_correct) %>%
  ggplot(aes(x = number_correct)) + 
  geom_bar(aes(fill = incentive)) + 
  #facet_wrap(~ incentive, labeller=labeller(incentive = labels)) +
  theme_igray() +
  #guides(fill=FALSE) +
  labs(x = 'Number of Questions Answered Correctly', y = 'Number of People', title = 'Number of people by each number correctly answered questions' )
  #mean(correct.answer))#, .groups = 'keep') 
...
misinfo.combined %>%
  ...
  theme_igray() +
  guides(fill=FALSE) +
  labs(x = 'Number of Questions Answered Correctly', y = 'Number of People', title = 'Number Correctly Answered Questions vs. Treatment')
  #mean(correct.answer))#, .groups = 'keep') 
```
...
```{r}
ogmodel <- lm(divide.coding ~ pid.binary, misinfo.combined)
model <- lm(divide.coding ~ pid.binary + incentive + pid.binary*incentive, misinfo.combined)

stargazer(ogmodel, type = "text")
```
...
yes a low p-value means that we can reject the null hypothesis of homoskedasticity.Thus both models are heteroskedastic.  
...
```{r}
coeftest(ogmodel, vcov. = vcovHC(model, type = 'HC1'))
